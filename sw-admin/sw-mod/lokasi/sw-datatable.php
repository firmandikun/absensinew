    ...
    while ($aRow = mysqli_fetch_array($rResult)){$no++;
      extract($aRow);
        ...
        for ($i=1 ; $i<count($aColumns) ; $i++){
            ...
            if($modifikasi =='Y'){
                $btn_update = '<a href="javascript:void(0)" onClick="location.href='.$onlick[0].'lokasi&op=update&id='.epm_encode($aRow['lokasi_id']).''.$onlick[1].';" class="table-action table-action-primary btn-tooltip" data-toggle="tooltip" title="Edit">
