    ...
    $(document).ready(function() {
        ...
        table = $('.datatable').DataTable({
           ...
           "fnDrawCallback": function () {
            ...
            "ajax": {
                "url": "./sw-mod/izin/sw-datatable.php",
                "type": "POST",
                "data": {
...
$(document).on('click', '.btn-add', function(){
  ...
  $('.form-add').submit(function (e) {
    ...
    e.preventDefault();
    $.ajax({
        url  : './sw-mod/izin/sw-proses.php?action=add',
        type: "POST",
...
$(document).on('click', '.btn-update', function(){
    ...
    $('.modal-add').modal('show');
    $('.modal-title').html('Update Izin');
    $('.tanggal').prop('readonly', true);
    $('.tanggal-selesai').prop('readonly', true);
    $.ajax({
        type: 'POST',
        url  : './sw-mod/izin/sw-proses.php?action=get-data-update',
        data: {id:id},
        dataType:'json',
        success: function(response) {
            $('.id').val(response.izin_id);
            $('.user').val(response.user_id);
            $('.tanggal').val(response.tanggal_mulai);
 ...
 $(document).on('click', '.btn-status', function(){
    ...
    $.ajax({
        type: "POST",
        url: "./sw-mod/izin/sw-proses.php?action=setujui",
  ...
  $(document).on('click', '.btn-status-tolak', function(){
    ...
    $.ajax({
        type: "POST",
        url: "./sw-mod/izin/sw-proses.php?action=tolak",
        data:{id:id,status:status},
        success:function(data){ 
            if (data == 'success') {
                swal({title: 'Berhasil!', text: 'Data berhasil simpan!', icon: 'success', timer: 2500,});
