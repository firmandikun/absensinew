...
if(!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])){
  ...
  exit;
    ...
    $aColumns = ['kartu_nama_id', 'nama','foto','active'];
    $sIndexColumn = "kartu_nama_id";
    $sTable = "kartu_nama";
    $gaSql['user'] = DB_USER;
    ...
    $sOrder = "ORDER BY kartu_nama_id DESC";
    if (isset($_GET['iSortCol_0']))
    {
        ...
        if ($sOrder == "ORDER BY kartu_nama_id DESC")
        {
            $sOrder = "ORDER BY kartu_nama_id DESC";
        }
    }
      ...
      extract($aRow);
        ...
        for ($i=1 ; $i<count($aColumns) ; $i++){
            ...
            if($aRow['active'] =='Y'){
                ...
                <input type="checkbox" class="btn-active active'.$aRow['kartu_nama_id'].'" data-id="'.$aRow['kartu_nama_id'].'" data-active="Y" checked>
                    <span class="custom-toggle-slider rounded-circle" data-label-off="No" data-label-on="Yes"></span>
              </label>';
            }else{
                 $active = '<label class="custom-toggle" style="display:inline-block">
                <input type="checkbox" class="btn-active active'.$aRow['kartu_nama_id'].'"  data-id="'.$aRow['kartu_nama_id'].'"  data-active="N">
                <span class="custom-toggle-slider rounded-circle" data-label-off="No" data-label-on="Yes"></span>
              </label>';
            }

            if($modifikasi =='Y'){
                $btn_update = '<a href="javascript:void(0)" class="table-action table-action-primary btn-update btn-tooltip" data-toggle="tooltip"  data-placement="right" title="Edit" data-id="'.$aRow['kartu_nama_id'].'" data-foto="'.strip_tags($aRow['foto']).'" data-name="'.strip_tags($aRow['nama']).'">
            ...
            if($hapus =='Y'){
             $btn_hapus ='<a href="javascript:void(0)" class="table-action table-action-delete btn-tooltip btn-delete" data-toggle="tooltip" data-placement="right" title="Hapus" data-id="'.epm_encode($aRow['kartu_nama_id']).'" data-name="'.strip_tags($aRow['nama']).'">
