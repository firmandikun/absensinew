    ...
    $(document).ready(function() {
        ...
        table = $('.datatable').DataTable({
           ...
           // "order": [[1, 'desc']],
            ...
            "ajax": {
                "url": "./sw-mod/posisi/sw-datatable.php",
                "type": "POST"
            },
...
$(document).on('click', '.btn-add', function(){
    ...
    $('.modal-title').html('Tambah Jabatan baru');
  ...
  /** Tambah Jam Kerja  */
    ...
    function submitForm_Add() { 
        ...
        $.ajax({    
            type : 'POST',
            url  : './sw-mod/posisi/sw-proses.php?action=add',
            data : data,
            cache: false,
            async: false,
            beforeSend: function() { 
    ...
    $(document).on('click', '.btn-update', function(){
        ...
        $('.id').val(id);
        $('.posisi-nama').val(name);
        $('.modal-title-name').html(name);
        $('.modal-add').modal('show');
        $('.modal-title').html('Update Posisi');
        //$(".form-add").trigger("reset");
    });
    ...
    var name = $(this).attr("data-name");
      ...
      .then((value) => {
        if(value) {
            loading();
            $.ajax({  
                 url:'./sw-mod/posisi/sw-proses.php?action=delete',
                 type:'POST',    
                 data:{id:id},  
                success:function(data){ 
                    if (data == 'success') {
