...
}elseif(!empty($_GET['pegawai']) && !empty($_GET['bulan']) && !empty($_GET['tahun'])){
    $pegawai    = strip_tags($_GET['pegawai']);
    $bulan      = strip_tags($_GET['bulan']);
    $tahun      = strip_tags($_GET['tahun']);
...
$query_data = "SELECT * FROM kunjungan $filter ORDER BY kunjungan_id ASC";
    ...
    <div class="row">
      ...
      <div class="col-md-12">
          <table class="datatable mt-3">
              ...
              <tbody>';
              while ($data_kunjungan = $result_data->fetch_assoc()){$no++;

                $query_pegawai ="SELECT nip,nama_lengkap,posisi_nama FROM user 
                INNER JOIN posisi ON user.posisi_id = posisi.posisi_id WHERE user.user_id='$data_kunjungan[user_id]'";
                ...
                <tr>
                  ...
                  <td>'.tanggal_ind($data_kunjungan['date']).'</td>
          ...
          if(!empty($_GET['pegawai'])){
            $query_hadir  = "SELECT kunjungan_id FROM kunjungan $filter AND status='Y'";
            $hadir        = $connection->query($query_hadir);

            echo'
            <table style="margin-top:10px">
              <tr>
              <td width="200">Total Kunjungan : <span class="badge badge-success">'.$hadir->num_rows.'</span></td>
            </table>';
          }
         echo' 
      </div>
