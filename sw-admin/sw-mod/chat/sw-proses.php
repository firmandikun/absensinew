...
if(htmlentities($_GET['id'] =='1')){
  $query_frendlist = "SELECT chat_list.user_id,user.nama_lengkap,user.avatar,user.time,user.status FROM chat_list
  INNER JOIN user ON chat_list.user_id = user.user_id WHERE chat_list.admin_id='$current_user[admin_id]' ORDER BY chat_list.datetime DESC";
  ...
  if($result_frendlist->num_rows > 0){
    while($data_frendlist = $result_frendlist->fetch_assoc()){
      ...
      $query_pesan ="SELECT pesan,datetime FROM chat WHERE status_user='N' AND user_id='$data_frendlist[user_id]' AND admin_id='$current_user[admin_id]' ORDER BY chat_id DESC LIMIT 1";
...
case 'search-frendlist':
  ...
  $query_frendlist = "SELECT user_id,nama_lengkap,avatar,time,status FROM user WHERE nama_lengkap like '%".$search."%'";
  $result_frendlist = $connection->query($query_frendlist);
  if($result_frendlist->num_rows > 0){
    while($data_frendlist = $result_frendlist->fetch_assoc()) {
...
$query_frendlist = "SELECT admin_id FROM chat_list WHERE user_id='$id'";
$result_frendlist = $connection->query($query_frendlist);

if($result_frendlist->num_rows > 0){
  /** Update jika sudah ada di daftar */
  $update="UPDATE chat_list SET datetime='$date $time_sekarang' WHERE user_id='$id' AND admin_id='$current_user[admin_id]'"; 
...
$query_frendlist = "SELECT chat_list.user_id,chat_list.admin_id,user.nama_lengkap,user.avatar,user.time FROM chat_list
INNER JOIN user ON chat_list.user_id = user.user_id WHERE chat_list.user_id='$id' AND chat_list.admin_id='$current_user[admin_id]'";
$result_frendlist = $connection->query($query_frendlist);
  if($result_frendlist->num_rows > 0){
    $data_frendlist = $result_frendlist->fetch_assoc();
    
    $data['user_id']        = anti_injection($data_frendlist["user_id"]);
...
$query_pesan ="SELECT chat.*,user.nama_lengkap,user.avatar FROM chat 
INNER JOIN user ON user.user_id = chat.user_id WHERE chat.user_id='$id' AND chat.admin_id='$current_user[admin_id]' ORDER BY chat.chat_id ASC";
$result_pesan = $connection->query($query_pesan);
if($result_pesan->num_rows > 0) {
