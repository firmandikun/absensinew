...
$(document).on('click', '.btn-supervisor-status', function(){
    ...
    $.ajax({
        type: 'POST',
        url: './sw-mod/cuti/sw-proses.php?action=update_supervisor_status',
        data: {id: id, supervisor_status: supervisor_status},
        success: function(data) {
            if (data == 'success') {
                swal({title: 'Berhasil!', text: 'Status supervisor berhasil diupdate cuti', icon: 'success', timer: 2500,});
                loadData();
            } else {
                swal({title: 'Gagal!', text: data, icon: 'error', timer:2500,});
            }
