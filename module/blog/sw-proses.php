...
$query_artikel="SELECT artikel_id,judul,domain,foto,deskripsi,kategori,date FROM artikel WHERE active='Y' $filter ORDER BY artikel_id DESC LIMIT 8";
...
echo'<div class="show-blog" style="position-relative">
  ...
  echo'
  <div class="col-12">
    <div class="alert alert-secondary mt-3">Saat ini belum ada artikel!</div>
  </div>';
}
...
$showLimit = 8;
$query_artikel="SELECT artikel_id,judul,domain,foto,deskripsi,kategori,date FROM artikel WHERE active='Y' AND artikel_id < $id $filter ORDER BY artikel_id DESC LIMIT $showLimit";
$result_artikel = $connection->query($query_artikel);
if($result_artikel->num_rows > 0){
      while ($data_artikel = $result_artikel->fetch_assoc()){
        $artikel_id = anti_injection($data_artikel['artikel_id']);
